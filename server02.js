...
api1.get('/storeServices/', async (req,res) =>{
    
    const sql = "SELECT DISTINCT service_name FROM services;";

    try {
        const [rs, fields] = await db.execute(sql);                             // SELECT文を実行して結果をretとfieldsに保存
        res.json(rs);                                                           // retの内容をJSONとして返却
    } catch(e) {
        res.status(400).json({error: '店舗のサービス情報取得に失敗しました。'});   // レスポンスのステータスを400に設定してからエラーメッセージのJSONを返却
        logger.error(e);                                                        // ログにもerror()として例外を出力
    }
});

api1.get('/serviceCount/', async (req,res) =>{
    
    const sql = "SELECT COUNT(DISTINCT service_name) FROM services;";

    try {
        const [rs, fields] = await db.execute(sql);                             // SELECT文を実行して結果をretとfieldsに保存
        res.json(rs);                                                           // retの内容をJSONとして返却
    } catch(e) {
        res.status(400).json({error: '店舗のサービス数取得に失敗しました。'});      // レスポンスのステータスを400に設定してからエラーメッセージのJSONを返却
        logger.error(e);                                                         // ログにもerror()として例外を出力
    }
});

api1.get('/name-services/', async (req,res) =>{
    const sql = "SELECT * FROM services NATURAL JOIN stores WHERE ";
    const queryParams = req.query;                  // クエリパラメータを取得
    const keys = Object.keys(queryParams);          // クエリパラメータ名の配列を取得
    let responseText = '';                          // パラメータ名と値を動的に使用
    ...
    for (let i=0; i<keys.length; i++) {
        ...
        if(i < keys.length - 1){                //次のキーが存在するかチェック
            ...
            responseText += `${key} LIKE '%${value}%' GROUP BY store_name;`;
        }
    }
